...
<c:if test="${empty shoppingCar }">
    <jsp:forward page="/CartServlet?op=findShoppingCar&uid=${user.uid}"></jsp:forward>
</c:if>
    ...
    <div id="templatemo_wrapper">
        ...
        <div id="templatemo_header">
            ...
            <div id="header_right">
                ...
                <p>
                    <c:if test="${!empty user }">
                        <a href="${pageContext.request.contextPath }/OrderServlet?op=findMyOrder">我的订单</a> |
                    </c:if>
                </p>
            </div>
                            ...
                            <c:forEach items="${categories }" var="category" varStatus="vs">
                                <c:if test="${vs.index !=0}">
                                    <c:if test="${vs.index != fn:length(categories)-1 }">
                                        <li>
                                            <a href="${pageContext.request.contextPath }/user/UserProductServlet?op=byCid&cid=${category.cid}">${category.cname}</a>
                                        </li>
                                    </c:if>
                                </c:if>
                                <c:if test="${vs.index==0 }">
                        ...
                        <table style="width: 700px;border-style:1px solid " border="1">
                            ...
                            <c:forEach items="${shoppingCar.shoppingItems}" var="item">
                                <tr id="tr${item.product.pid}">
                                    <td><input type="checkbox" class="ids" name="ids" value="${item.product.pid}"
                                               checked></td>
                                    <td><img
                                            src="${pageContext.request.contextPath}/images/product/${item.product.imgurl}"
